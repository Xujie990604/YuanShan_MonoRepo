...
var createTaskSchema = zod.z.object({
  ...
  // 关联的角色 ID（可选）
  // 日期字段验证
  dueDate: zod.z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "\u65E5\u671F\u683C\u5F0F\u5FC5\u987B\u4E3A YYYY-MM-DD").optional(),
  dueTime: zod.z.string().regex(/^([0-1]\d|2[0-3]):[0-5]\d$/, "\u65F6\u95F4\u683C\u5F0F\u5FC5\u987B\u4E3A HH:mm").optional(),
  recurrence: zod.z.object({
    type: zod.z.enum(["DAILY", "WEEKLY", "MONTHLY"]),
    interval: zod.z.number().int().min(1).default(1),
    daysOfWeek: zod.z.array(zod.z.number().int().min(0).max(6)).optional(),
    dayOfMonth: zod.z.number().int().min(1).max(31).optional()
  }).optional()
});
var updateTaskSchema = zod.z.object({
  title: zod.z.string().min(1, "\u4EFB\u52A1\u6807\u9898\u4E0D\u80FD\u4E3A\u7A7A").max(64, "\u4EFB\u52A1\u6807\u9898\u4E0D\u80FD\u8D85\u8FC7 64 \u4E2A\u5B57\u7B26").optional(),
  description: zod.z.string().max(1e3, "\u4EFB\u52A1\u8BE6\u60C5\u4E0D\u80FD\u8D85\u8FC7 1000 \u4E2A\u5B57\u7B26").nullable().optional(),
  ...
  dueDate: zod.z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "\u65E5\u671F\u683C\u5F0F\u5FC5\u987B\u4E3A YYYY-MM-DD").nullable().optional(),
  dueTime: zod.z.string().regex(/^([0-1]\d|2[0-3]):[0-5]\d$/, "\u65F6\u95F4\u683C\u5F0F\u5FC5\u987B\u4E3A HH:mm").nullable().optional(),
  recurrence: zod.z.object({
    type: zod.z.enum(["DAILY", "WEEKLY", "MONTHLY"]),
    interval: zod.z.number().int().min(1).default(1),
    daysOfWeek: zod.z.array(zod.z.number().int().min(0).max(6)).optional(),
...
var createRoleSchema = zod.z.object({
  name: zod.z.string().min(1, "\u89D2\u8272\u540D\u79F0\u4E0D\u80FD\u4E3A\u7A7A").max(10, "\u89D2\u8272\u540D\u79F0\u4E0D\u80FD\u8D85\u8FC7 10 \u4E2A\u5B57\u7B26"),
...
var updateRoleSchema = zod.z.object({
  name: zod.z.string().min(1, "\u89D2\u8272\u540D\u79F0\u4E0D\u80FD\u4E3A\u7A7A").max(10, "\u89D2\u8272\u540D\u79F0\u4E0D\u80FD\u8D85\u8FC7 10 \u4E2A\u5B57\u7B26").optional(),
